...
const API_DETALLE = "http://localhost:8081/api/detalle-carrito";
    ...
    async function cargarCarrito() {
        try{
            const res = await fetch(`${API_CARRITO}?idUsuario=${usuario.idUsuario}`, { method: "POST" });
    ...
    function renderCarrito() {
        ...
        carrito.detalles.forEach(det => {
            cont.innerHTML += `
                <div class="item">
                    <div>
                        <div class="name">${det.nombreProducto ?? "Producto"}</div>
                        <small>Precio: $${det.subtotal / det.cantidad}</small>
                    </div>

                    <div class="controls">
                        <button class="btn-sub" onclick="cambiarCantidad(${det.idProducto}, ${det.cantidad - 1})">-</button>
                        <span>${det.cantidad}</span>
                        <button class="btn-add" onclick="cambiarCantidad(${det.idProducto}, ${det.cantidad + 1})">+</button>
                        <button class="btn-delete" onclick="eliminar(${det.idProducto})">🗑</button>
                    </div>
                </div>
            `;
        });
    ...
    async function cambiarCantidad(idProducto, nuevaCantidad) {
        ...
        await fetch(`${API_DETALLE}/${carrito.idCarrito}/actualizar?idProducto=${idProducto}&nuevaCantidad=${nuevaCantidad}`, {
            method: "PUT"
        });
